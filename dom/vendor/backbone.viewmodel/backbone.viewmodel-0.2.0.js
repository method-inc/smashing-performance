...
(function() {
  ...
  (function(Backbone) {
    var AttrBinding, CssBinding, EachBinding, OnBinding, ScrollBinding, StyleBinding, TextBinding, ValueBinding, VisibleBinding, axisMap, innerMap, outerMap;
    ...
    Backbone.Binding['attr'] = AttrBinding = (function(_super) {
      ...
      AttrBinding.prototype.initialize = function(domAttribute, attribute) {
        this.domAttribute = domAttribute;
        this.attribute = attribute;
      };

      AttrBinding.prototype.onModelChange = function() {
        var val;
        val = this.viewModel.get(this.attribute);
        return $(this.node).attr(this.domAttribute, val);
      };
    ...
    Backbone.Binding['val'] = ValueBinding = (function(_super) {
      ...
      ValueBinding.prototype.start = function() {
        ...
        return ValueBinding.__super__.start.apply(this, arguments);
      };
    ...
    Backbone.Binding['style'] = StyleBinding = (function(_super) {

      __extends(StyleBinding, _super);

      function StyleBinding() {
        return StyleBinding.__super__.constructor.apply(this, arguments);
      }

      StyleBinding.prototype.initialize = function(styleProperty, attribute) {
        this.styleProperty = styleProperty;
        this.attribute = attribute;
      };

      StyleBinding.prototype.onModelChange = function() {
        var val;
        val = this.viewModel.get(this.attribute);
        return $(this.node).css(this.styleProperty, val);
      };

      return StyleBinding;

    })(Backbone.Binding);
    Backbone.Binding['on'] = OnBinding = (function(_super) {

      __extends(OnBinding, _super);

      function OnBinding() {
        return OnBinding.__super__.constructor.apply(this, arguments);
      }

      OnBinding.prototype.initialize = function() {
        var args, callback, events;
        events = arguments[0], callback = arguments[1], args = 3 <= arguments.length ? __slice.call(arguments, 2) : [];
        this.events = events;
        this.callback = callback;
        this.args = args;
      };

      OnBinding.prototype.start = function() {
        return $(this.node).on(this.events, this.onViewChange);
      };

      OnBinding.prototype.onViewChange = function(event) {
        var callback, _ref;
        ...
        callback = (function() {
          var _ref1;
          if ((_ref = (_ref1 = this.viewModel[this.callback]) != null ? _ref1 : this.viewModel.get(this.callback)) != null) {
            return _ref;
          } else {
            throw new Error("'on' binding can't find callback '" + this.callback + "' for '" + this.events + "'");
          }
        }).call(this);
        return callback.apply(this.viewModel, this.args);
      };

      OnBinding.prototype.stop = function() {
        return $(this.node).off(this.events, this.onViewChange);
      };

      return OnBinding;

    })(Backbone.Binding);
    Backbone.Binding['each'] = EachBinding = (function(_super) {
    ...
    return Backbone.Binding['scroll'] = ScrollBinding = (function(_super) {

      __extends(ScrollBinding, _super);

      function ScrollBinding() {
        this.tweenScroll = __bind(this.tweenScroll, this);

        this.startTween = __bind(this.startTween, this);

        this.stopTween = __bind(this.stopTween, this);

        this.onViewChange = __bind(this.onViewChange, this);
        return ScrollBinding.__super__.constructor.apply(this, arguments);
      }

      ScrollBinding.prototype.initialize = function(attribute, axis, speed) {
        this.attribute = attribute;
        this.axis = axis != null ? axis : 'y';
        this.speed = speed != null ? speed : 0.5;
        this.speed = isNaN(speed) ? parseInt(speed, 10) : speed;
        this.scrollProp = axisMap[this.axis];
        this.outerProp = outerMap[this.axis];
        this.innerProp = innerMap[this.axis];
        this.target = 0;
        this.last = void 0;
        return this.scrollTimeout = void 0;
      };

      ScrollBinding.prototype.start = function() {
        $(this.node).on('scroll', this.onViewChange);
        return ScrollBinding.__super__.start.apply(this, arguments);
      };

      ScrollBinding.prototype.onViewChange = function() {
        var percent;
        if (this.scrollTimeout != null) {
          return;
        }
        percent = this.getScrollPercent();
        return this.viewModel.set(this.attribute, percent, {
          scrollBinding: true
        });
      };

      ScrollBinding.prototype.onModelChange = function(vm, val, options) {
        var max;
        if (options != null ? options.scrollBinding : void 0) {
          return;
        }
        val = this.viewModel.get(this.attribute);
        max = this.node[this.outerProp] - this.node[this.innerProp];
        this.target = val * max;
        return this.startTween();
      };

      ScrollBinding.prototype.getScrollPercent = function() {
        var max, val;
        val = this.node[this.scrollProp];
        max = this.node[this.outerProp] - this.node[this.innerProp];
        if (max > 0) {
          return val / max;
        } else {
      ...
      ScrollBinding.prototype.stopTween = function() {
        clearTimeout(this.scrollTimeout);
        this.scrollTimeout = void 0;
        return this.last = void 0;
      };

      ScrollBinding.prototype.startTween = function() {
        this.stopTween();
        return this.tweenScroll();
      };

      ScrollBinding.prototype.tweenScroll = function() {
        var current, tween;
        current = this.node[this.scrollProp];
        tween = current + (this.target - current) * this.speed;
        this.node[this.scrollProp] = tween;
        if (Math.abs(current - this.target) <= 2 || Math.abs(this.node[this.scrollProp] - this.last) <= 2) {
          this.stopTween();
          this.node[this.scrollProp] = this.target;
          return;
        }
        this.last = current;
        return this.scrollTimeout = setTimeout(this.tweenScroll, 30);
      };

      ScrollBinding.prototype.stop = function() {
        ScrollBinding.__super__.stop.apply(this, arguments);
        return $(this.node).off('scroll', this.onViewChange);
      };
